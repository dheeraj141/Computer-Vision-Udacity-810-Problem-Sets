...
class KDNode(Node):
    ...
    def __init__(self, data=None, left=None, right=None, axis=None,
            sel_axis=None, dimensions=None, descriptor_index = None):
    ...
    def add(self, point, index1):
        ...
        while True:
            ...
            if current.data is None:
                current.data = point
                current.descriptor_index = index1
                return current

            # split on self.axis, recurse either left or right
            if point[current.axis] < current.data[current.axis]:
                if current.left is None:
                    current.left = current.create_subnode(point, index1)
                    return current.left
                else:
                    current = current.left
            else:
                if current.right is None:
                    current.right = current.create_subnode(point, index1)
    ...
    #@require_axis
    def create_subnode(self, data, index1):
        """ Creates a subnode for the current node """

        return self.__class__(data,descriptor_index = index1,
